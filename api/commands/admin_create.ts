...
export default class AdminCreate extends BaseCommand {
    ...
    async run(): Promise<void> {
        ...
        const fullNameInput = this.fullName ?? (await this.prompt.ask("Nom complet de l'administrateur"))
        const passwordInput = this.password ?? (await this.prompt.secure('Mot de passe (min. 8 caractères)'))
