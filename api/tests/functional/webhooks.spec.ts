...
test.group('Webhooks Controller - HelloAsso', (group) => {
  ...
  test('helloasso webhook ignores duplicate webhook for already succeeded payment', async ({
    client,
  }) => {
...
test.group('Webhooks Controller - HelloAsso with mock', (group) => {
  ...
  test('webhook should update payment and registrations on successful verification', async ({
    client,
    assert,
  }) => {
    const user = await User.create({ email: 'user@example.com' })
