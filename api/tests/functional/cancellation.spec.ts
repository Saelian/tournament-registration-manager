...
test.group('Cancellation - Refund eligibility', (group) => {
  ...
  test('refund eligibility returns eligible when within deadline', async ({ client, assert }) => {
    ...
    const response = await client.get(`/api/payments/${payment.id}/refund-eligibility`).loginAs(user)
  ...
  test('refund eligibility returns not eligible when deadline passed', async ({ client, assert }) => {
    const user = await User.create({ email: 'user@example.com' })
    const tournament = await Tournament.create({
      name: 'Test Tournament',
  ...
  test('cannot check eligibility for payment of another user', async ({ client }) => {
    ...
    const response = await client.get(`/api/payments/${payment.id}/refund-eligibility`).loginAs(user2)
