...
test.group('Tables | CRUD', (group) => {
    ...
    test('update table', async ({ client, assert }) => {
        ...
        const response = await client.put(`/admin/tables/${table.id}`).withGuard('admin').loginAs(admin).json({
            name: 'Updated Name',
            quota: 64,
        })
    ...
    test('delete table', async ({ client, assert }) => {
        ...
        const response = await client.delete(`/admin/tables/${table.id}`).withGuard('admin').loginAs(admin)
