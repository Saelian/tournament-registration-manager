...
export function useTableFilters<T extends object>(data: T[], options: UseTableFiltersOptions<T> = {}) {
  const { filters: filterConfigs = [], persistToUrl = false, searchKeys = [], initialFilters = {} } = options

  const [searchParams, setSearchParams] = useSearchParams()

  // Compute initial state lazily in useState to avoid re-computation
  const [search, setSearchState] = useState(() => (persistToUrl ? parseUrlFilters(searchParams).search : ''))
