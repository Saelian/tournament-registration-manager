...
export function CsvExportModal({
  ...
  const handleColumnToggle = (key: string) => {
    setColumns((prev) => prev.map((col) => (col.key === key ? { ...col, included: !col.included } : col)))
  }
            ...
            <div className="space-y-2 max-h-64 overflow-y-auto border-2 border-foreground p-3 rounded-lg">
              {columns.map((col) => (
                <div key={col.key} className="flex items-center gap-3">
                  <Checkbox id={`col-${col.key}`} checked={col.included} onChange={() => handleColumnToggle(col.key)} />
                  <Input
                    value={col.label}
          ...
          {showPresentOnlyOption && (
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-3 p-3 border-2 border-foreground rounded-lg bg-green-50">
                <Checkbox id="present-only" checked={presentOnly} onChange={() => setPresentOnly(!presentOnly)} />
