...
const STATUS_CONFIGS: Record<StatusType, { colors: Record<string, string>; labels: Record<string, string> }> = {
    payment: {
...
export function StatusBadge({ status, type, icon, className }: StatusBadgeProps) {
    ...
    return (
        <span
            className={cn('inline-flex items-center gap-1 px-2 py-0.5 text-xs font-bold border', colorClass, className)}
        >
            {icon}
            {label}
        </span>
