...
export function SortableDataTable<T extends object>({
  ...
  // Use filters hook
  const { search, filters, filteredData, hasActiveFilters, setSearch, setFilter, clearFilter, clearAllFilters } =
  ...
  const { sortState, sortedData, toggleSort } = useTableSort(filteredData, {
    initialSort: initialSort ? { column: initialSort.column, direction: initialSort.direction } : undefined,
  })
    ...
    <div className={cn('space-y-4', className)}>
      ...
      {showToolbar && (
        <div className="flex flex-col sm:flex-row gap-3">
          {searchable && (
            <SearchInput value={search} onChange={setSearch} placeholder={searchPlaceholder} className="sm:w-64" />
          )}
