...
export function SponsorForm({ initialData, onSubmit, onCancel, isLoading }: SponsorFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-4 bg-card p-6 neo-brutal">
      <h2 className="text-xl font-bold mb-4">{initialData ? 'Modifier le sponsor' : 'Nouveau sponsor'}</h2>
      ...
      <div className="flex items-center space-x-2">
        <Controller
          ...
          control={control}
          render={({ field }) => (
            <Checkbox id="isGlobal" checked={field.value} onChange={(e) => field.onChange(e.target.checked)} />
          )}
        />
        ...
        <div className="border-t-2 border-foreground pt-4 mt-4">
          ...
          <div className="flex flex-wrap gap-2">
            {initialData.tables.map((table) => (
              <span key={table.id} className="bg-secondary text-sm px-2 py-1 border border-foreground rounded">
                {table.name}
              </span>
            ))}
          </div>
