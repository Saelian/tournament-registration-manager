...
import { UserPlus, Copy, Check, AlertTriangle, Loader2, Search, X, UsersIcon, Ban, Clock } from 'lucide-react'
...
export function AdminRegistrationForm({ open, onOpenChange }: AdminRegistrationFormProps) {
  ...
  const totalAmount = tables.filter((t) => selectedTableIds.includes(t.id)).reduce((sum, t) => sum + t.price, 0)
  ...
  const isPlayerEligible = (table: Table): { eligible: boolean; reason: string | null } => {
    ...
    if (table.allowedCategories && table.allowedCategories.length > 0) {
      if (
        ...
        !table.allowedCategories.includes(selectedPlayer.category as (typeof table.allowedCategories)[number])
      ) {
          ...
          <div className="space-y-4 mt-4">
            ...
            <div className="space-y-2">
              <label className="flex items-start space-x-2 cursor-pointer">
                <Checkbox checked={bypassRules} onChange={(e) => setBypassRules(e.target.checked)} className="mt-0.5" />
                <div>
                  <span className="text-sm font-medium">Ignorer certaines règles</span>
                  <p className="text-xs text-muted-foreground mt-0.5">
                    Permet d'outrepasser la limite de 2 tableaux par jour et le quota maximum de joueurs.
                  </p>
                </div>
              </label>
            </div>
            ...
            {bypassRules && (
              <div className="bg-amber-100 dark:bg-amber-900/30 border-2 border-amber-500 p-3 flex items-start gap-2 text-sm">
                ...
                <p>
                  L'inscription sera créée même si le joueur dépasse la limite de 2 tableaux par jour ou si le tableau a
                  atteint son quota maximum. Les règles de classement restent appliquées.
                </p>
              </div>
            )}
        ...
        {step === 'payment' && (
          <div className="space-y-4 mt-4">
            <div className="bg-muted/50 border-2 border-foreground p-3">
              ...
              <p className="text-sm text-muted-foreground">{selectedTableIds.length} tableau(x) sélectionné(s)</p>
            </div>
