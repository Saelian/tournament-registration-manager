        ...
        <div className="grid grid-cols-[1fr_auto] gap-4 mb-4 items-end">
          ...
          <div className="grid grid-cols-2 gap-3 pb-2">
            {directTables.map((table) => (
              <div key={table.id} className="flex items-center justify-between gap-2 bg-card px-3 py-2 neo-brutal">
          ...
          <div className="flex flex-col items-end gap-2 pb-2">
            <div className="text-right">
              <div className="text-sm text-muted-foreground">Total {waitlistTables.length > 0 && '(hors attente)'}</div>
              <div className="text-3xl font-bold">{formatPrice(totalPrice)} €</div>
            </div>
            <Button size="lg" onClick={onSubmit} disabled={isSubmitting} className="shadow-shadow neo-brutal-hover">
              {isSubmitting ? 'Redirection...' : "Valider l'inscription"}
            </Button>
          </div>
        </div>

        {/* Bloc d'information contextuel */}
