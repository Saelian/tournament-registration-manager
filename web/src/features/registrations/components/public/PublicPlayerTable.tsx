...
function MobilePlayerCard({ player, showTableColumn }: { player: AggregatedPublicPlayer; showTableColumn: boolean }) {
    return (
 ...
 */
function aggregateByPlayer(registrations: PublicRegistrationData[], dayFilter?: string): AggregatedPublicPlayer[] {
...
export function PublicPlayerTable({
    ...
    const aggregatedPlayers = useMemo(() => aggregateByPlayer(registrations, selectedDay), [registrations, selectedDay])
                ...
                {paginatedMobilePlayers.length === 0 ? (
                    ...
                    <div className="space-y-3">
                        {paginatedMobilePlayers.map((player) => (
                            <MobilePlayerCard key={player.licence} player={player} showTableColumn={showTableColumn} />
                        ))}
                    </div>
                )}
