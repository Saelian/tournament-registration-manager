...
export function AdminTableListPage() {
  ...
  const filteredTables = useMemo(() => {
    ...
    if (search) {
      ...
      result = result.filter(
        (table) =>
          table.name.toLowerCase().includes(searchLower) || table.referenceLetter?.toLowerCase().includes(searchLower)
      )
    }

    // Apply filters
    Object.entries(filters).forEach(([key, value]: [string, FilterValue]) => {
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 animate-on-load animate-slide-up">
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        <SearchInput value={search} onChange={setSearch} placeholder="Rechercher un tableau..." className="sm:w-64" />
        ...
        {filteredTables.length === 0 && (
          <div className="text-center p-8 bg-secondary border-2 border-dashed border-foreground">
            <p className="font-bold text-muted-foreground">
              {hasActiveFilters ? 'Aucun tableau ne correspond aux critères.' : 'Aucun tableau créé.'}
            </p>
            {hasActiveFilters && (
      ...
      <Dialog open={!!tableToDelete} onOpenChange={(open) => !open && setTableToDelete(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Êtes-vous sûr de vouloir supprimer le tableau "{tableToDelete?.name}" ? Cette action est irréversible.
            </DialogDescription>
          </DialogHeader>
