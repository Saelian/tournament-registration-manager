...
export function TablePrizesSection({ tableId, value = [], onChange }: TablePrizesSectionProps) {
  ...
  const prizes = tableId ? connectedPrizes : (value.map((p, i) => ({ ...p, id: i })) as TablePrize[])
  ...
  const handleAdd = () => {
    const nextRank = Math.max(0, ...prizes.map((p: TablePrize) => p.rank)) + 1
    setFormState({
      rank: nextRank,
      prizeType: 'cash',
      cashAmount: '',
    ...
    <div className="border-t-2 border-foreground pt-4 mt-4">
      ...
      <div className="space-y-2">
            ...
            {editingId === prize.id ? (
              <div className="flex-1 flex flex-wrap items-center gap-2">
                ...
                <select
                  ...
                  onChange={(e) => setFormState({ ...formState, prizeType: e.target.value as 'cash' | 'item' })}
              ...
              <>
                ...
                <div className="flex gap-1">
                  <Button type="button" variant="secondary" size="sm" onClick={() => handleEdit(prize)}>
        ...
        {prizes.length === 0 && !isAdding && (
          <p className="text-sm text-muted-foreground text-center py-2">Aucun prix configuré pour ce tableau</p>
        )}
      </div>
    </div>
  )
