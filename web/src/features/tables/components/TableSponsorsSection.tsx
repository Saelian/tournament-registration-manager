...
import { useSponsors, type Sponsor } from '../../sponsors'
...
export function TableSponsorsSection({ tableId, value = [], onChange }: TableSponsorsSectionProps) {
  ...
  useEffect(() => {
    ...
    setSelectedIds(currentSponsors.map((s: TableSponsor) => s.id))
  }, [currentSponsors])
        ...
        <div className="space-y-2">
          {allSponsors && allSponsors.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              {allSponsors.map((sponsor: Sponsor) => (
                <label
                  key={sponsor.id}
                  className={`flex items-center gap-2 p-2 rounded border-2 cursor-pointer transition-colors ${
                    selectedIds.includes(sponsor.id) ? 'border-primary bg-primary/10' : 'border-foreground bg-secondary'
                  }`}
                >
                  <input
                    type="checkbox"
                  ...
                  {sponsor.isGlobal && <span className="text-xs bg-yellow-300 px-1 rounded">Global</span>}
                </label>
              ))}
            </div>
          ) : (
        ...
        <div>
          {currentSponsors.length > 0 ? (
            <div className="flex flex-wrap gap-2">
              {currentSponsors.map((sponsor: TableSponsor) => (
                <span
                  key={sponsor.id}
