  ...
  return (
    <div
      className={cn(
        ...
        isSelected ? 'border-primary shadow-[4px_4px_0px_0px_var(--primary)]' : 'border-foreground shadow-shadow'
      )}
      onClick={handleClick}
    >
          ...
          <div className="flex items-center gap-2 mb-2 flex-wrap">
            ...
            {variant === 'public' && player && (
              <>
                ...
                {!isEligible && !isAlreadyRegistered && !hasTimeConflict && <Badge variant="error">Inéligible</Badge>}
              </>
            )}
          ...
          {table.allowedCategories && table.allowedCategories.length > 0 && (
            <div className="mt-2 flex flex-wrap gap-1">
              ...
              {table.allowedCategories.map((cat) => (
                <span key={cat} className="bg-secondary text-xs px-2 py-0.5 border border-foreground">
                  {cat}
                </span>
              ))}
            </div>
          ...
          {(table.totalCashPrize > 0 || table.prizes?.length > 0) && (
            <div className="mt-2 flex items-center gap-2 text-sm">
              ...
              {table.totalCashPrize > 0 ? (
                <span className="font-bold text-yellow-700">{formatPrice(table.totalCashPrize)} € de dotation</span>
              ) : (
                <span className="">
          ...
          {table.sponsors?.length > 0 && (
            <div className="mt-1 flex flex-wrap gap-1">
              ...
              {table.sponsors.map((sponsor) => (
                <span key={sponsor.id} className="bg-blue-100 text-xs px-2 py-0.5 border border-blue-300">
                  {sponsor.name}
                </span>
              ))}
            </div>
