        ...
        {step === 'email' ? (
          <form className="space-y-4 mt-4" onSubmit={handleSubmitEmail(onEmailSubmit)}>
            <div>
              ...
              <Input id="login-email" type="email" autoComplete="email" {...registerEmail('email')} className="mt-1" />
              {emailErrors.email && <p className="mt-1 text-sm text-destructive">{emailErrors.email.message}</p>}
            </div>
